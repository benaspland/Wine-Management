...
console.log(`\n=== TESTING WITH SIMPLIFIED CONSUMPTION FORMULA ===\n`);
console.log(`Loaded ${wines.length} wines (${totalBottles} bottles total)\n`);
...
for (let year = currentYear; year < currentYear + 100 && loopIterations < maxLoopIterations; year++) {
  ...
  for (let deliverySlot = 0; deliverySlot < 2; deliverySlot++) {
    ...
    if (shouldDeliver && cases.length > 0) {
      ...
      if (loopIterations <= 20 || loopIterations % 5 === 0) {
        console.log(`[${year}-${String(month).padStart(2, '0')}] Delivered ${totalDelivered} bottles (${cases.length} wines)`);
      }
    }
  }
}
...
const unscheduledBottles = Object.values(remaining).reduce((a, b) => a + b, 0);

console.log(`Loop iterations: ${loopIterations}`);
console.log(`Total scheduled: ${totalScheduled} / ${totalBottles} (${((totalScheduled/totalBottles)*100).toFixed(1)}%)`);
console.log(`Unscheduled: ${unscheduledBottles} bottles`);
