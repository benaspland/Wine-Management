...
console.log(`Total remaining: ${totalRemaining} / ${totalBottles}`);
console.log(`Loop iterations: ${loopIterations}`);

if (totalRemaining > 0) {
  console.log(`\nUnscheduled wines (${wines.filter(w => remaining[w.id] > 0).length}):`);
  wines.filter(w => remaining[w.id] > 0).slice(0, 20).forEach(w => {
